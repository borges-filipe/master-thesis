...
This thesis will explore a hallucination reviser (a method for correcting captions) and a hallucination detection benchmark (the new dataset containing real labeled hallucinations). These contributions could make existing VLM captions more reliable and provide new tools for future research.
...
\begin{itemize}
  ...
  \item Model: LLaVA (Large Language and Vision Assistant), an end-to-end trained multimodal model connecting a vision encoder to an LLM \cite{liuVisualInstructionTuning2023}. LLaVA is an appropriate choice for three primary reasons. First, as an open-source model, it grants full access to internal logits and token probabilities, which is a strict requirement for the Visual Contrastive Decoding method \cite{lengMitigatingObjectHallucinations2024}. Second, VCD is specifically designed to correct the over-reliance on language priors characteristic of LLMs, such as the one utilized as the decoder in LLaVA \cite{lengMitigatingObjectHallucinations2024}. Third, using LLaVA ensures that results are comparable with the original VCD study where this model was utilized as a representative model to demonstrate performance trajectories \cite{lengMitigatingObjectHallucinations2024}.
\end{itemize}
...
The thesis will be carried out over a period of six months. The following Gantt chart (Figure \ref{fig:schedule}) outlines the estimated schedule for the various phases of the thesis:
